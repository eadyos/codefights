/*
https://codefights.com/arcade/code-arcade/time-river/EaPEmzu6CYRuc4X2C


You're a pretty busy billionaire, and you often fly your personal Private Jet to remote places. Usually travel doesn't 
bother you, but this time you are unlucky: it's New Year's Eve, and since you have to fly halfway around the world, 
you'll probably have to celebrate New Year's Day in mid-air!

Your course lies west of your current location and crosses several time zones. The pilot told you the exact schedule: it 
is known that you will take off at takeOffTime, and in minutes[i] after takeoff you will cross the ith border between time 
...
The take off time in the 24-hour format "HH:MM". It is guaranteed that the given time is valid. The "00:00" time
 corresponds to the midnight of 31st of December / 1st of January.

[input] array.integer minutes

A strictly increasing array of integers. minutes[i] stands for the number of minutes from the take off to crossing the ith time 
zone border.
...
int newYearCelebrations(String takeOffTime, List<Integer> minutes) {
    
    def (hour, mins) = takeOffTime.split(":")*.toInteger()
    def midnight = 24*60
    def time = (hour * 60 + mins) ?: midnight - 1
    
    def maxMinute = minutes.max() ?: 0
