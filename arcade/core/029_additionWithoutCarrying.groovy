...
int additionWithoutCarrying(int param1, int param2) {
    ...
    String big = param1 > param2 ? param1 : param2
    String small = big.toInteger() == param1 ? param2 :  param1
    int sizeDiff = big.size() - small.size()
    small = "0" * sizeDiff + small
    (0..<big.size()).sum{
        def factor = 10 ** it
        ((big.reverse()[it].toInteger() +
                small.reverse()[it].toInteger()
        ) * (10 ** it)) % (10 ** (it+1))
    }
}
