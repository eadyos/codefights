/*
https://codefights.com/arcade/code-arcade/mirror-lake/kGeuCkJNbqczCCqgg

You are given an array of integers that you want distribute between several groups. The first group should contain numbers
from 1 to 104, the second should contain those from 104 + 1 to 2 * 104, ..., the 100th one should contain numbers from
99 * 104 + 1 to 106 and so on.
...
int numbersGrouping(List<Integer> a) {

    def valueSet = [] as Set
    a.each{
        valueSet << Math.floor(it-1/10000)
    }
    a.size() + valueSet.size()
