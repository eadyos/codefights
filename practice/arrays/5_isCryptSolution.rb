...
letters and digits, solution. The array crypt will contain three non-empty strings that follow the
structure: [word1, word2, word3], which should be interpreted as the word1 + word2 = word3
cryptarithm.

If crypt, when it is decoded by replacing all of the letters in the cryptarithm with digits using
the mapping in solution, becomes a valid arithmetic equation containing no numbers with leading
zeroes, the answer is true. If it does not become a valid arithmetic solution, the answer is false.
...
Return true if the solution represents the correct solution to the cryptarithm crypt, otherwise
return false.
=end

def isCryptSolution(crypt, solution)

    lookup = solution.group_by{|a| a[0]}
    translation = crypt.map{|word|
        word.chars.map{|c|
            lookup[c][0][1]
        }.join
    }
    translation.all?{|word|
        !word =~ /0[1-9]/
    } &&
    translation[0].to_i + translation[1].to_i == translation[2].to_i
