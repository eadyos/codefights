/*
https://codefights.com/arcade/code-arcade/mirror-lake/RpoP4Aqa5mbmC8koC
...
int mostFrequentDigitSum(int n) {
    ...
    def valueGroups = values.collect{
        digitSum(it)
    }.groupBy{
        it
    }
    def maxSize = valueGroups.max{it.value.size()}.value.size()
    valueGroups.findAll{
...
def digitSum(n){
    n.toString().split("").sum{it as int}
}
