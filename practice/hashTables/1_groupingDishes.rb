...
def groupingDishes(dishes)
    ...
    dishes.each{|dish|
        name = dish[0]
        dish[1..-1].each{|ingredient|
            if !ingredientDishes[ingredient]
                ingredientDishes[ingredient] = []
            end
            ingredientDishes[ingredient] << name
        }
    }

    ingredientDishes.keys.select{|key|
        ingredientDishes[key].size > 1
    }.sort.map{|key|
        ingredientDishes[key].sort.unshift(key)
    }
