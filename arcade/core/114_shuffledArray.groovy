/*
https://codefights.com/arcade/code-arcade/sorting-outpost/s4BEFMcpLdGbjX9KX
...
position of the original array which was a bad idea. Now he needs to cope with the second task, sorting the original 
...
def shuffledArray(List<Integer> a) {
  
    def index = (0..<a.size()).find{n->
        a[n] == (0..<a.size()).sum{ it == n ? 0 : a[it] }
    }
    a.remove(index)
    a.sort()    
