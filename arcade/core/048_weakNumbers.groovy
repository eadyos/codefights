...
def devisorCache = [:]
def weaknessCache = [:]
List<Integer> weakNumbers(int n) {

    def weaknesses = (1..n).collect{
        weakness(it)
    }
    int max = weaknesses.max()
    int count = weaknesses.count{it == max}
    [max, count]
}

def weakness(n){
    int nDevisors = devisors(n)
    if(weaknessCache[n] == null){
        int weakness = (1..<n).count{
            int d = devisors(it)
            d > nDevisors
        }
        weaknessCache[n] = weakness
    }
    weaknessCache[n]
}

def devisors(n){
    if(devisorCache[n] == null){
        devisorCache[n] = (1..n).count{n % it == 0}
    }
    devisorCache[n]
}
